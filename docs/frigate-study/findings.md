...
## Batches 46-50 (PRs 10321-5433)

Fourth fan-out, 5 worktree-isolated agents x ~159 PRs = 792 PRs in one run (the bulk of the remaining tail). Net: **19 HAVE / 771 N/A / 3 GAP / 0 fixes.** Frigate 0.13-era (early 2024): the WebUI redesign wave plus go2rtc/birdseye/hwaccel/ML/SQLite internals. A near-total N/A desert for the Nurby backend, as expected this deep.

**GAPs (2 issues filed, 1 minor logged):**
- **#9708 ONVIF skip non-video profiles -> issue Eshpelin/nurby#101.** `_probe_device` builds the stream URI from `profile_tokens[0]` unconditionally (`services/discovery/onvif.py:327-328`), no filter for a profile carrying a video encoder config. Cameras whose first ONVIF profile is audio/metadata-only get a non-video stream URL surfaced for ingest (`cameras.py:236-247`). P3 latent; behavioral change wants a test + docker verify, not a blind one-liner.
- **#6319 recording-segment delete cleanup -> issue Eshpelin/nurby#102.** `Event.recording_id` is a bare nullable UUID with **no ForeignKey/ondelete** (`shared/models.py:671-673`), while 20+ sibling FK columns are properly constrained. `RetentionManager` recording deletes (`retention.py:353,416`) leave dangling `recording_id`s; the "jump to clip" link 404s. P2 data integrity; needs an Alembic migration (FK + `ondelete="SET NULL"` + null-out backfill), so issue not blind merge.
- **#9710 last-clip-for-camera+label endpoint** — minor convenience-feature gap; Nurby has filtered/ordered recordings queries (`recordings.py:169,203`) but no single-shot "latest clip" route. Low value, logged in ledger only.

**Notable verified HAVEs (Nurby already correct, often more robustly):** credential hygiene — never leaked in API responses (`_camera_to_response` masks password, `cameras.py:50`) and never in stream URLs/logs (creds injected at connect via `build_auth_url`, `stream.py:70`; error log uses bare url, `:443`) (#8757/#9189/#8399/#5800); retention robustness — `_remove_file` treats a missing file as success and still prunes the DB row (`retention.py:69,339`) (#8531/#8433); numeric guards — audio RMS `+1e-12` no log10/dBFS div-by-zero (`audio_worker.py:172`), aspect divisions guarded (`har_actions.py:100,156`), box coords `int()` before `cv2.rectangle` (`recording_annotate.py:122`) (#8447/#7425/#8388); frame checks use `is None` not truthiness (`stream.py:473`) (#6968); WS sends wrapped in broad except + BrokenPipe/ConnectionReset (`ws.py:102,302`) (#7554/#7525/#7556); event queries bounded `limit le=200` (`events.py:48,144`) (#7108); path safety via `resolve_inside` + UUID paths + ext sanitize (`paths.py`, `persons.py:782`) (#8089/#8090); storage-stats `isdir`+fallback before `psutil.disk_usage` (`system.py:129`) (#8275); ONVIF GetPresets faults return `[]` (`onvif.py:655`) (#6497).

**All backend-adjacent Frigate fixes N/A by architecture** (re-confirmed across all 5 slices): Flask `jsonify`/`make_response` status quirks, SQLite WAL/vacuum/migration-backup/db-timeout tuning, ZMQ/multiprocess queue races + SHM frame leaks, python-onvif/zeep PTZ object init, autotracking RelativeMove zoom-spaces, CV motion-mask/improved_motion pipeline, ReviewSegment maintainer, YAML config validators, MQTT, nginx/CORS/cookie/CSRF auth — none map onto Nurby's Postgres+Alembic / async single-app / FastAPI Bearer / raw-httpx-ONVIF / VLM-perception design.

### Cursor note
After this run the cursor reaches **PR 5433**. Roughly ~200 of Frigate's oldest merged PRs remain below it (the 2020-2023 founding era: original detection/recording/MQTT/config scaffolding, almost entirely pre-FastAPI-comparison and N/A). One more small fan-out (or a single batch) reaches the bottom of the 4058.

## Batches 51-55 (PRs 5430-1) — FINAL: sweep complete

Fifth and last fan-out: 5 parallel agents over 602 PRs (#5430 → #1), Frigate's entire founding era (2019-2023): the original Flask + peewee/SQLite app, MQTT, MJPEG, CV motion/zone pipeline, gevent, multiprocess+SHM, YAML config, early recording/clips. Net: **11 HAVE / 590 N/A / 1 GAP→FIX / 1 fix shipped.** With this run the 4058-PR backlog is fully triaged (cursor reaches #1).

**FIX shipped — PR #104 (the one actionable finding in the whole tail):** camera credentials were not URL-encoded in `build_auth_url` (`services/ingestion/stream.py:70`). Raw `username:password` went into the netloc, so a password containing `@ : / # ? %` corrupted URL parsing and resolved the wrong host — the camera silently never connected. Frigate fixed the same class in #4259/#4057/#4646 (`escape_special_characters`). Fix: `quote(..., safe="")` on both credential parts; simple creds unchanged. All 5 injection call sites route through this one function. Verified in docker + 7 unit tests (`tests/test_build_auth_url.py`).

**Verified HAVEs (Nurby already correct):** #5013 events `status` filter covers in_progress (`events.py:137`); #5056 storage-stats `isdir`+fallback (`system.py:129`); #3969/#3246 all outbound httpx timeout-bounded; #2543 `image/jpeg` uniform; #4016 ffmpeg-extract guards `returncode!=0 or not stdout` + `wait_for` (`analyzer.py:357,345`); #1293 media 404 on missing file/row (`recordings.py:96,78`); #1136 all ffmpeg/ffprobe via argv-list `create_subprocess_exec`, never `shell=True`; #427 retention deletes by DB `camera_id`+timestamp (not Frigate's name-keyed path dict; the original Frigate disk-leak bug class can't occur here).

**Everything else N/A by architecture**, re-confirmed at the source: Flask/gevent/peewee/SQLite plumbing, multiprocess+SHM frame handling, ZMQ-era event maintainer, CV motion/zone/region/mask/tracking pipeline, MQTT topics/switches, YAML config dataclasses, edgetpu/Coral/Nvidia/VAAPI/RPi hwaccel presets, go2rtc/birdseye/WebRTC/MJPEG restream, nginx/HA-ingress, the original React frontend, docs, deps, CI. None map onto Nurby's FastAPI + async + Postgres/Alembic + Bearer-auth + raw-httpx-ONVIF + VLM-perception design.
