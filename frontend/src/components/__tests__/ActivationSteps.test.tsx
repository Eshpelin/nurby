...
vi.mock("@/lib/auth", () => ({ useAuth: () => ({ authFetch: mocks.fetch, token: "test-token" }) }));
vi.mock("next/link", () => ({
...
describe("ActivationSteps", () => {
  ...
  it("labels a synthetic test and still refuses to show verified", async () => {
    ...
    expect(screen.getByRole("button", { name: /I received this alert/ })).toBeDisabled();
  });
  ...
  it("surfaces the server's reason when confirm is refused", async () => {
    mocks.fetch.mockImplementation(async (url: string, init?: RequestInit) => {
      if (url.includes("/activation/evidence")) {
        return { ok: true, json: async () => ({ event_id: "e1", recording_id: "clip1", seek_seconds: 0 }) };
      }
      if (init?.method === "POST") {
    ...
    fireEvent.click(await screen.findByRole("button", { name: "Review this test clip" }));
    fireEvent.playing(await screen.findByLabelText("Recording for this test event"));
    fireEvent.click(screen.getByRole("button", { name: /I received this alert/ }));
    await waitFor(() => expect(screen.getByRole("alert")).toHaveTextContent(/No delivered test event yet/));
  });

  it("plays the exact recording and binds confirmation to its event", async () => {
    mocks.fetch.mockImplementation(async (url: string, init?: RequestInit) => {
      if (url.includes("/activation/evidence")) return { ok: true, json: async () => ({ event_id: "test-event", recording_id: "exact-clip", seek_seconds: 0 }) };
      if (init?.method === "POST") return { ok: true, json: async () => view({ verified: true }) };
      return list(view({ rule_id: "r1", steps: [
        { key: "configured", done: true, synthetic: false },
        { key: "tested", done: true, synthetic: false },
        { key: "confirmed", done: false, synthetic: false },
      ] }));
    ...
    render(<ActivationSteps goal="entrance" cameraId={null} />);
    expect(await screen.findByRole("button", { name: /I received this alert/ })).toBeDisabled();
    fireEvent.click(screen.getByRole("button", { name: "Review this test clip" }));
    const video = await screen.findByLabelText("Recording for this test event");
    expect(video).toHaveAttribute("src", "/api/recordings/exact-clip/stream?token=test-token");
    expect(screen.getByRole("button", { name: /I received this alert/ })).toBeDisabled();
    fireEvent.playing(video);
    fireEvent.click(screen.getByRole("button", { name: /I received this alert/ }));
    await waitFor(() => expect(mocks.fetch).toHaveBeenCalledWith("/api/auth/me/activation/confirm", expect.objectContaining({
      body: JSON.stringify({ goal: "entrance", event_id: "test-event" }),
    })));
  });

  it("a playback error keeps confirmation disabled", async () => {
    mocks.fetch.mockImplementation(async (url: string) => {
      if (url.includes("/activation/evidence")) return { ok: true, json: async () => ({ event_id: "e1", recording_id: "clip1", seek_seconds: 0 }) };
      return list(view({ rule_id: "r1", steps: [
        { key: "configured", done: true, synthetic: false },
        { key: "tested", done: true, synthetic: false },
        { key: "confirmed", done: false, synthetic: false },
