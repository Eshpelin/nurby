...
const CHIPS = [
  ...
  { label: "What happened today?", q: "What happened today?" },
...
export function AskComposerCard() {
  ...
  return (
    <div className="mb-4">
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            onBlur={() => { setTimeout(mention.close, 100); setTimeout(() => setFocused(false), 150); }}
            onKeyDown={(e) => {
            ...
            placeholder="Ask Nurby about your cameras…  (@ to tag)"
            className="w-full px-3 py-2 rounded-md bg-card border border-border text-sm focus:outline-none focus:border-accent"
          />
        </div>
        <button
          type="button"
      ...
      {/* Suggestions reveal on focus, so they don't sit as permanent chrome. */}
      {focused && !q && (
        <div className="flex flex-wrap gap-1.5 mt-2">
          {CHIPS.map((c) => (
            <button
              key={c.label}
              type="button"
              onMouseDown={(e) => { e.preventDefault(); go(c.q); }}
